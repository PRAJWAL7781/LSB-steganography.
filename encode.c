...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    ...
    if (argv[4] != NULL)
    {
        char *dot = strrchr(argv[4], '.');  // find last '.'
        if (dot && strcmp(dot, ".bmp") == 0)
        {
            // Case 1: already has .bmp
            printf("Stego image name provided: %s\n", argv[4]);
            encInfo->stego_image_fname = argv[4];
        }
        else
        {
            // Case 2: has other extension OR no extension -> strip and add .bmp
            static char stego_with_ext[100];
            if (dot)
                *dot = '\0';  // remove extension by replacing '.' with null char
    ...
    {
        // Case 3: no argv[4], use default name
...
Status encode_magic_string_length(int len, EncodeInfo *encInfo)
{
    ...
    printf("Magic string length is stored.\n");
    return e_success;
}
...
Status encode_secret_file_extn_len(int extn_len, EncodeInfo *encInfo)
{
    ...
    printf("Secret file extension length is stored.\n");
    return e_success;
}
...
Status encode_secret_file_extn(const char *extn, EncodeInfo *encInfo)
{
    ...
    printf("Secret file extension is stored\n");
    return e_success;
}
...
Status encode_secret_file_size(long file_size, EncodeInfo *encInfo)
{
    ...
    fwrite(buffer, 1, 32, encInfo->fptr_stego_image);
    printf("Secret file size is stored.\n");
    return e_success;
}
...
Status encode_secret_file_data(EncodeInfo *encInfo)
{
    ...
    printf("Secret file data is stored.\n");
    return e_success;
}
...
Status copy_remaining_img_data(FILE *fptr_src, FILE *fptr_dest)
{
    ...
    printf("Copy remaining image data is stored.\n");
    return e_success;
}
